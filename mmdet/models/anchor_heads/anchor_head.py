...
from mmdet.datasets import map_box_back
...
class AnchorHead(nn.Module):
    ...
    def __init__(self,
        ...
        for anchor_base in self.anchor_base_sizes:
            self.anchor_generators.append(
                AnchorGenerator(anchor_base, anchor_scales, anchor_ratios, times=anchor_times))
        if self.anchor_times is None:
            self.num_anchors = len(self.anchor_ratios) * len(self.anchor_scales)
        else:
            self.num_anchors = len(self.anchor_ratios) * int(np.sum(np.array(self.anchor_times)**2))
    ...
    def get_anchors(self, featmap_sizes, img_metas):
        ...
        # since feature map sizes of all demo are the same, we only compute
    ...
    def get_bboxes(self, cls_scores, bbox_preds, img_metas, cfg,
        ...
        for img_id in range(len(img_metas)):
            ...
            im_info = img_metas[img_id]['im_info']
            proposals = self.get_bboxes_single(cls_score_list, bbox_pred_list,
                                               mlvl_anchors, img_shape,
                                               scale_factor, cfg, rescale, im_info)
    ...
    def get_bboxes_single(self,
        ...
        if rescale:
            ...
            mlvl_bboxes = map_box_back(mlvl_bboxes, crop[2], crop[0], im_scale)

...
def worker_func(num_images_3dce, num_slices,  model_cls, model_kwargs, checkpoint, dataset,
    ...
    with torch.no_grad():
        while True:
            idx = idx_queue.get()
            data = dataset[idx]
            result = model(**_data_func(data, num_images_3dce, num_slices, gpu_id))
            result_queue.put((idx, result))


def parallel_test(cfg,
                  model_cls,
                  model_kwargs,
                  checkpoint,
                  dataset,
    ...
    num_images_3dce = cfg.cfg_3dce.num_images_3dce
    num_slices = cfg.cfg_3dce.num_slices
    workers = [
        ctx.Process(
            target=worker_func,
            args=(num_images_3dce, num_slices, model_cls, model_kwargs, checkpoint, dataset,
...
def _data_func(data, num_images_3dce, num_slices, device_id):
    data = reorganize_data(data, num_images_3dce, num_slices)
